...
/**
 * Handles the Course Data metabox.
 */
final class CourseDataMetabox {
	...
	public function render( \WP_Post $post ): void {
		...
		<div class="lw-lms-course-data">
			...
			<div class="lw-lms-attachments-list" id="lw-lms-attachments">
				<?php
			...
			<button type="button" class="button lw-lms-add-attachment" id="lw-lms-add-attachment">
				<?php esc_html_e( 'Add Attachment', 'lw-lms' ); ?>
			</button>

			<input type="hidden" name="lw_lms_attachments" id="lw-lms-attachments-data" value="<?php echo esc_attr( wp_json_encode( $attachments ) ); ?>" />
		</div>
		<?php
	}
	...
	public function save( int $post_id ): void {
		if ( ! isset( $_POST['lw_lms_course_data_nonce'] ) ) {
			return;
		}

		if ( ! wp_verify_nonce( sanitize_key( $_POST['lw_lms_course_data_nonce'] ), 'lw_lms_course_data' ) ) {
			return;
		}
		...
		// Save attachments.
		if ( isset( $_POST['lw_lms_attachments'] ) ) {
			$attachments = json_decode( sanitize_text_field( wp_unslash( $_POST['lw_lms_attachments'] ) ), true );
			if ( is_array( $attachments ) ) {
				$sanitized = [];
				foreach ( $attachments as $attachment ) {
					$sanitized[] = [
