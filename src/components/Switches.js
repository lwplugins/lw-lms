...
export function SwitchList( { children } ) {
	return <ul className="lw-lms-switches">{ children }</ul>;
}
...
export function SwitchItem( {
	...
	const id = useInstanceId( SwitchItem, 'lw-lms-switch' );
	const classes = [
		'lw-lms-switch',
	...
	return (
		<li className={ classes.filter( Boolean ).join( ' ' ) }>
			...
			<div className="lw-lms-switch__body">
				<div className="lw-lms-switch__head">
					<label className="lw-lms-switch__title" htmlFor={ id }>
						{ title }
					</label>
					{ badge }
				</div>
				{ help && (
					<p className="lw-lms-switch__help" id={ `${ id }-help` }>
						{ help }
					</p>
				) }
