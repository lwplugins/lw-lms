 ...
 */
import LmsMark from '../components/LmsMark';
 ...
 */
export default function SideNav( { tabs, groups, current, meta = {} } ) {
		...
		<aside className={ `lw-admin-sidebar ${ isOpen ? 'is-open' : '' }` }>
			<div className="lw-admin-sidebar__head">
				<a
					...
					href={ `#${ tabs[ 0 ].id }` }
					aria-label={ __( 'LW LMS home', 'lw-lms' ) }
				>
					<LmsMark />
					<strong>LW LMS</strong>
				</a>
			...
			<div className="lw-admin-sidebar__foot">
				...
				>
					...
					<span className="lw-admin-sidenav__label">
						{ __( 'Documentation', 'lw-lms' ) }
					</span>
					<Icon icon={ external } size={ 16 } />
					<span className="screen-reader-text">
						{ __( '(opens in a new tab)', 'lw-lms' ) }
					</span>
				</a>
			</div>
		</aside>
